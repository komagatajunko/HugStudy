...
SELECT store_table.store_name, goods_table.goods_name, stock_table.quantity FROM stock_table
...
SELECT AVG(price) AS "平均価格" FROM goods_table;

-- 5. 店舗コード（'EA03'）の店舗に関連する在庫情報（商品コード、在庫数）を取得して下さい。
SELECT goods_code, quantity FROM stock_table
WHERE store_code = 'EA03';

-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、価格=4500円、更新日付=本日日付」のデータを追加して下さい。。※実行後の「データ出力」画面も貼付すること。
INSERT INTO goods_table (goods_code, goods_name, price, update_day) VALUES ('M001', 'マフラー', '4500', '2026-01-16');
...
INSERT INTO store_table (store_code, store_name, store_nameAbc, update_day) VALUES ('EA09','新宿店','SHINJUKUTEN','2012-08-01');
INSERT INTO store_table (store_code, store_name, store_nameAbc, update_day) VALUES ('WE03','梅田店','UMEDETEN','2013-02-01');
INSERT INTO store_table (store_code, store_name, store_nameAbc, update_day) VALUES ('WE04','福岡店','FUKUOKATEN','2014-05-01');
